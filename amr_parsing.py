...
def main():
    ...
    # test deterministic oracle 
        ...
        train_instances = preprocess(amr_file,START_SNLP=False,INPUT_AMR=args.amrfmt,PRP_FORMAT=args.prpfmt)
        #print len(train_instances) #1253 INSTANCES
        #sys.exit()
        if args.add: 
            train_instances = train_instances + preprocess(args.add,START_SNLP=True,INPUT_AMR=args.amrfmt,PRP_FORMAT=args.prpfmt)
            #print len(train_instances) #NAO ENTRA AQUI
            #sys.exit()
        if args.dev: 
            dev_instances = preprocess(args.dev,START_SNLP=False,INPUT_AMR=args.amrfmt,PRP_FORMAT=args.prpfmt)
            #print len(dev_instances) # 140 INSTANCES
            #sys.exit()
